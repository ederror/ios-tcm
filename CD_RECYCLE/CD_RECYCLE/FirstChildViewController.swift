...
class FirstChildViewController: UIViewController { // 가로 쓰레기통
    ...
    override func viewDidLoad() {
        ...
        let targets = self.getNearTC(current: LO.location)
    ...
    func getNearTC(current: NMGLatLng) -> ([NMGLatLng], [String : [String]]) {
        let TCrequest: NSFetchRequest<TrashCan> = TrashCan.fetchRequest()
        let TCfetchResult = PersistenceManager.shared.fetch(request: TCrequest)
